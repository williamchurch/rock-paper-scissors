        ...
        if computerMove == 0:
            if userMove == "rock":
                ...
                await message.channel.send(
                    "user used paper computer used rock. Player wins!"
                )
            else:
                await message.channel.send(
                    "user used scissors, computer used rock. Computer wins!"
                )
            ...
            if userMove == "paper":
                ...
                await message.channel.send(
                    "user used scissors, computer used paper. Player wins!"
                )
            else:
                await message.channel.send(
                    "user used rock, computer used paper. Computer wins!"
                )
            ...
            if userMove == "scisscors":
                ...
                await message.channel.send(
                    "user used rock computer used scissors. Player wins!"
                )
            else:
                await message.channel.send(
                    "user used paper, computer used scissors. Computer wins!"
                )
...
async def on_message(message):
    ...
    if message.content.startswith("$rps "):
        ...
        if amount_of_mentions == 0:
            ...
            await message.channel.send(
                "Too many mentions in message, expected either one or none"
            )
